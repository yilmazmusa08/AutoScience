...
def binary_classification(df, cv=5, target=None, models=['Logistic Regression', 'Random Forest',
    ...
    cb_scores_mean = {metric: round(np.mean(cb_scores[f'test_{metric}']), 2) for metric in metrics}
...
"""import pandas as pd
df = pd.read_csv('/home/firengiz/Belgeler/proje/automl/models/GOOG.csv')
print(df.info())
print(df['low'].nunique())"""