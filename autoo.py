...
async def run_models(request: Request, file: UploadFile = File(...), date: str = Form(None), target: str = Form(None), forecast: int = Form(None)):
    try:
        df = pd.read_csv(file.file)

        sonuc = create_model(df, date=date, forecast=forecast, target=target) if date else create_model(df, date=None,forecast=None,target=target)
